// Point this at your live Render backend (Dashboard -> your service -> URL at the top).
// Example: "https://googlemaps-lead-scraper.onrender.com"
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok && data.status === 'success') {
                ...
                downloadLink.href = `${API_BASE_URL}/download/${data.filename}`;
            } else {
    ...
    // Reset stop button state when loading area is hidden
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.target === loadingArea && loadingArea.classList.contains('hidden')) {
                  stopBtn.disabled = false;
                  stopBtn.innerText = "Stop Scraping";
            }
        });
    });
    observer.observe(loadingArea, { attributes: true, attributeFilter: ['class'] });
});